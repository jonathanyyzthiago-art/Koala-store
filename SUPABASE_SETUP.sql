...
drop policy if exists "admin can read own store config" on public.store_config;
create policy "admin can read own store config"
on public.store_config for select to authenticated
using (auth.uid() = user_id);

drop policy if exists "admin can insert own store config" on public.store_config;
create policy "admin can insert own store config"
on public.store_config for insert to authenticated
with check (auth.uid() = user_id);

drop policy if exists "admin can update own store config" on public.store_config;
